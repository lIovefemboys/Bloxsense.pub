...
function library:LoadConfig(config)
    ...
    if not table.find(configs, config) then return end
    ...
    for _, option in pairs(self.options) do
        if option.type ~= "button" and option.flag and not option.skipflag then
            local value = data[option.flag]
            if value ~= nil then
                if option.type == "toggle" then
                    option:SetState(value == 1, true)
                elseif option.type == "color" then
                    if type(value) == "table" and #value == 3 then
                        local color = Color3.new(value[1], value[2], value[3])
                        option:SetColor(color, true)
                        if option.trans and data[option.flag .. " Transparency"] then
                            option:SetTrans(data[option.flag .. " Transparency"], true)
                        end
                    end
                elseif option.type == "bind" then
                    if value ~= "none" then
                        option:SetKey(Enum.KeyCode[value] or Enum.UserInputType[value])
                    end
                elseif option.type == "list" then
                    if option.multiselect then
                        ...
                        if table.find(option.values, value) then
                ...
                elseif option.type == "box" then
                    option:SetValue(tostring(value), true)
                end
            end
        end
    end
...
function library:SaveConfig(config)
    ...
    -- Загружаем существующий конфиг, чтобы не потерять данные других опций
    ...
    for _, option in pairs(self.options) do
        if option.type ~= "button" and option.flag and not option.skipflag then
            ...
            elseif option.type == "bind" then
                if option.key and option.key ~= "none" then
                    Config[option.flag] = option.key
                else
                    Config[option.flag] = "none"
                end
            elseif option.type == "list" then
                if option.multiselect then
...
function library:GetConfigs()
    ...
    for _, v in ipairs(listfiles(self.foldername)) do
        if v:sub(#v - #self.fileext + 1) == self.fileext then
            local name = v:gsub(self.foldername .. "\\", ""):gsub(self.fileext, "")
...
function library:ExportToClipboard(config)
    ...
    if not table.find(configs, config) then
        return false, "Config not found: " .. config
    end
    
    local success, data = pcall(function()
        return readfile(self.foldername .. "/" .. config .. self.fileext)
    end)
    ...
    -- Копируем в буфер обмена (работает в Synapse X, Script-Ware, Krnl и др.)
    ...
    if not clipboardSuccess then
        return false, "Failed to copy to clipboard (executor doesn't support clipboard)"
    end
    
    return true, "Config exported to clipboard successfully!"
end

function library:ImportFromClipboard(config)
    if not config or config == "" then 
        return false, "Config name is empty"
    end
    ...
    local clipboardSuccess = pcall(function()
        if syn and syn.clipboard then
            clipboardData = syn.clipboard()
        elseif getclipboard then
            clipboardData = getclipboard()
        elseif toclipboard then
            clipboardData = toclipboard()
        elseif clipboard then
    ...
    if not clipboardSuccess or not clipboardData then
        return false, "Failed to read from clipboard"
    end
    
    local success, jsonData = pcall(function()
        return game:GetService("HttpService"):JSONDecode(clipboardData)
    end)
    
    if not success or not jsonData then
        return false, "Invalid config data in clipboard (not a valid JSON)"
    end
    ...
    local writeSuccess = pcall(function()
        writefile(self.foldername .. "/" .. config .. self.fileext, clipboardData)
    end)
    ...
    return true, "Config imported and loaded successfully!"
end
